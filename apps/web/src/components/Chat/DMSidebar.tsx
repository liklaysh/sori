...
export const DMSidebar: React.FC<DMSidebarProps> = ({
  ...
  return (
    <div className="w-64 h-full bg-sori-surface-main flex flex-col border-r border-sori-border-subtle animate-in fade-in duration-300 relative">
      <header className="h-14 border-b border-sori-border-subtle flex items-center px-4 shrink-0 shadow-sm bg-sori-surface-main">
        <h2 className="text-[11px] font-black uppercase tracking-[0.2em] text-sori-text-muted">Direct Messages</h2>
      </header>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-3 space-y-1">
        ...
        >
           <div className="w-8 h-8 rounded-lg bg-sori-surface-panel flex items-center justify-center border border-sori-border-subtle group-hover:border-sori-border-accent transition-all">
        ...
        {conversations.length === 0 ? (
          <div className="py-10 text-center px-4">
             <div className="w-12 h-12 bg-sori-surface-panel rounded-2xl flex items-center justify-center mx-auto mb-3">
            ...
            return (
              <div 
                ...
                className={cn(
                  ...
                  isActive ? 'bg-sori-surface-hover text-sori-text-strong font-bold' : 'text-sori-text-muted hover:bg-sori-surface-hover hover:text-sori-text-primary'
                )}
              >
                <div className="relative shrink-0">
                  ...
                  <div className={cn(
                    "absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-sori-surface-main",
         ...
         {livekitToken && (
          <div className="bg-sori-surface-panel px-3 py-2 border-t border-sori-border-subtle flex items-center gap-3 animate-in slide-in-from-bottom relative">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[10px] text-sori-text-muted font-bold truncate">Current Call</div>
            </div>
            ...
            <button 
              onClick={endCall} 
              className="p-1.5 bg-sori-error rounded-xl text-black hover:brightness-110 transition-all shadow-sm"
              title="Disconnect"
            >
          ...
          <div className="flex items-center gap-0.5">
            <div className="flex items-center relative h-8">
              <button 
                ...
                className={cn(
                  ...
                  isMuted ? 'text-sori-error hover:bg-sori-error-subtle' : 'text-sori-text-muted hover:text-white hover:bg-sori-surface-hover'
                )}
              >
            ...
            <div className="flex items-center relative h-8">
              <button 
                ...
                className={cn(
                  "w-8 h-8 flex items-center justify-center rounded-lg transition-all",
                  isDeafened ? 'text-sori-error hover:bg-sori-error-subtle' : 'text-sori-text-muted hover:text-white hover:bg-sori-surface-hover'
                )}
              >
                <div className="relative">
                   <Headphones className={cn("h-5 w-5", isDeafened ? "text-sori-error" : "text-sori-text-muted")} />
                   {isDeafened && (
                     <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-[2px] bg-sori-error rotate-45 rounded-full" />
                   )}
                </div>
              </button>
              <Popover>
