...
export const MemberSidebar: React.FC<MemberSidebarProps> = ({ onlineUsersSet }) => {
  ...
  const renderMember = (m: any, isOnline: boolean) => (
    <div 
      key={m.id} 
      className={`flex items-center gap-3 py-2 px-3 hover:bg-sori-surface-hover rounded-xl cursor-pointer group transition-all ${isOnline ? '' : 'text-sori-text-dim grayscale'}`}
    >
      <div className="relative flex-shrink-0">
        ...
        {isOnline && <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-sori-accent-secondary rounded-full border-2 border-sori-surface-main shadow-sm"></div>}
      </div>
  ...
  return (
    <aside className="h-full w-64 bg-sori-surface-main border-l border-sori-border-subtle z-40 flex flex-col shrink-0">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-6">
        <section>
          <h3 className="px-2 text-[10px] uppercase font-black text-sori-text-dim mb-3 flex items-center gap-2">
