...
export const ServerSidebar: React.FC<ServerSidebarProps> = ({ 
  ...
  return (
    <aside className="fixed left-0 top-0 h-full flex flex-col z-50 bg-sori-surface-panel w-20 items-center py-4 space-y-4 border-r border-sori-border-subtle overflow-x-hidden overflow-y-auto no-scrollbar capitalize">
      {/* Home / Community Button */}
      <div 
        onClick={() => setActiveModule('community')}
        className={`w-12 h-12 flex items-center justify-center rounded-[1.5rem] shadow-lg cursor-pointer transition-all active:scale-95 group relative flex-shrink-0 border-none ${activeModule === 'community' ? 'bg-sori-accent-primary text-sori-text-on-primary' : 'bg-sori-surface-base text-sori-text-muted hover:text-sori-accent-primary hover:rounded-xl'}`}
      >
      ...
      <div className="flex-1 flex flex-col items-center space-y-3 w-full">
        ...
        <div 
          ...
          className={`w-12 h-12 flex items-center justify-center rounded-xl transition-all cursor-pointer group relative flex-shrink-0 ${activeModule === 'dm' ? 'bg-sori-accent-primary text-sori-text-on-primary shadow-lg' : 'bg-sori-surface-base text-sori-text-muted hover:text-sori-accent-primary'}`}
        >
