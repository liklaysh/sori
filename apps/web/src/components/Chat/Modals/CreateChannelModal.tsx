  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-md bg-sori-surface-panel rounded-[2.5rem] p-8 shadow-3xl border border-sori-border-subtle animate-in zoom-in-95 duration-300">
        <h2 className="text-2xl font-black mb-6 text-white text-center tracking-tight text-[rgb(219,222,225)]">Create Channel</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              <Hash className={type === "text" ? "text-white" : "text-sori-text-muted"} size={20} /> Text
            </div>
            <div 
            ...
            >
              <Volume2 className={type === "voice" ? "text-white" : "text-sori-text-muted"} size={20} /> Voice
            </div>
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-sori-text-muted pl-4">Channel Name</label>
            <input 
              autoFocus 
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={!name.trim()} className="bg-sori-accent-primary text-black px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-lg hover:brightness-110 active:scale-95 transition-all disabled:bg-sori-surface-disabled">CREATE</button>
          </div>
        </form>
      </div>
    </div>
